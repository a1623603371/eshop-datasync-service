 ...
 */
@Component
@RabbitListener(queues = "refresh-data-chang-queue")
public class RefershDataChangeQueueReceiver {
    @Autowired
...
public RefershDataChangeQueueReceiver(){
            ...
            while(true){
                if (!dimDataChangeMessageSet.isEmpty()){
                    for (String message:dimDataChangeMessageSet){
                        rabbitMQSender.send("refresh-aggr-data-chang-queue",message);
